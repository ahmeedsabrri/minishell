...
/*   Updated: 2023/08/25 18:32:43 by asabri           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//             add_token_back(&(init.token),newtoken(WORD,
//                 get_q(line,&init.i,line[init.i]),0,0));
//             init.double_quote = !init.double_quote;
//             init.single_quote = !init.single_quote;
//         }
...
//     }
    ...
    if ((init.singl_quote == 1|| init.double_quote == 1) || (init.op != init.cp))
		return (ft_putstr_fd("Syntax : Quote Unfound\n", 2), NULL);
	if (init.op != init.cp)
		return (ft_putstr_fd("Syntax : Need a parantes\n", 2), NULL);
